...
let out1Elem = null, out2Elem = null;
let monitorOn = false; // default OFF — don't hear yourself
...
function toggleMonitor() {
  ...
  btn.textContent = monitorOn ? '🔊 Hearing' : '🔇 Muted';
  btn.classList.toggle('on', monitorOn);
  if (out1Elem) out1Elem.muted = !monitorOn;
}
...
async function startMic() {
  try {
    ...
    // Primary output — muted by default (monitor off)
    dest1Node = audioCtx.createMediaStreamDestination();
    out1Elem = new Audio(); out1Elem.srcObject = dest1Node.stream;
    out1Elem.muted = !monitorOn;
    out1Elem.play();
